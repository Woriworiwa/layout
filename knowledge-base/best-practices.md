...
**Custom RxJS-based Store** (`@layout/canvas` library):
...
**UndoRedoService** (`@layout/canvas` library):
...
**Core data structure** (`@layout/models` library):
...
- Root wrapper uses constant `CANVAS_WRAPPER_ID` from `@layout/models`

### Serialization

**Export formats** (via `SerializationService` from `@layout/serialization`):
- HTML: Semantic markup
- CSS: Class-based or inline styles
- JSON: CanvasItem structure for persistence

**Serializers**: Exported from `@layout/serialization` library
